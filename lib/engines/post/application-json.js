		...
		try {
			...
			if (optionSpace && !optionSpace['enable']) {
				...
				ns.request.on('end', function () {
					...
					if (of(items, 'array')) {
						for (var i=0, I=options['postSize']>0 ? Math.min(items.length, options['postSize']) : items.length; i<I; i++) {
							var item = items[i];
						...
						if (options['postSize']>0) {
							var itemKeys = Object.keys(items);
							for (var i=options['postSize']; i<itemKeys.length; i++) {
