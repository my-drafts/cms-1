...
module.exports = function (nameSpace) {
	return new Promise(function (resolve, reject) {
		try {
			...
			ns.cookieDone = function () {
				ns.log('TRACE', ['EngineMetaCookie.cookieDone']);
				ns.response.setHeader('Set-Cookie', COOKIE_TO_SET);
			};
...
var __object4value = function (items, name, value, options) {
	try {
		return name && items.push(__cookie2serialized(String(name), String(value), options))>=0 ? value : false;
	}
	catch (e) {
		return false;
	}
