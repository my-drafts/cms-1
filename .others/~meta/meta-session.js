					...
					if (ns.SESSION_ID) {
						...
						if (id) {
							...
							storeIdChecker(id, ns.SESSION_EXPIRES).then(function (exists) {
								if (!exists) {
									ns.log('DEBUG', ['EngineMetaSession.sessionStart renew', id]);
									sessionStartReNew(ns, id).then(resolve, reject);
								}
								else {
									ns.log('DEBUG', ['engineMetaSession: sessionStart cookie ok', id]);
									ns.log('DEBUG', ['engineMetaSession: sessionStart getting sessionData']);
									storeDataGetter(id, ns.SESSION_EXPIRES).then(function (data) {
										ns.log('DEBUG', ['engineMetaSession: sessionStart getting sessionData ok', id, data]);
										ns.SESSION_ID = id;
										Object.freeze(ns.SESSION_ID);
										Object.assign(ns.SESSION_DATA, data);
										resolve(_id);
									}).catch(function () {
										ns.log('ERROR', ['engineMetaSession: sessionStart getting sessionData']);
							...
							ns.log('DEBUG', ['EngineMetaSession.sessionStart new']);
...
var sessionStartReNew = function (ns, id) {
	return new Promise(function (resolve, reject) {
		ns.log('DEBUG', ['EngineMetaSession.sessionStart renew ok', id]);
		/*
		 ns.cookie(ns.SESSION_NAME, '', { expires: -1 });
		 Object.freeze(ns.SESSION_ID);
		 reject('engineMetaSession: sessionStart with wrong sessionId in cookie');
		 */
		ns.cookie(ns.SESSION_NAME, id, { siteOnly: true });
		ns.SESSION_ID = id;
		Object.freeze(ns.SESSION_ID);
		resolve(id);
	});
